	...
	if ($INPN_url.StatusCode -eq "OK") {
		if ($inpn -eq "") {
			...
			if ($totalRecords -eq 0) {
				...
				$fin_data_inpn | Add-Content "./BDD/INPN/$code.csv"
	...
	if ($INATURALIST_url.StatusCode -eq "OK") {
		if ($inaturalist -eq "") {
			...
			if ($total_results -eq 0) {
				...
				$fin_data_inat | Add-Content "./BDD/INATURALIST/$code.csv"
	...
	if ($GBIF_url.StatusCode -eq "OK") {
		if ($GBIF -eq "") {
			...
			if ($count -eq 0)  {
				...
				$fin_data_gbif | Add-Content "./BDD/GBIF/$code.csv"
